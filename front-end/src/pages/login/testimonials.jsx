export const testimonials = [
    {
    name: "Alice",
    role: "ADHD Coach & Specialist",
    quote:"No one can multitask. We just have to focus on getting something done. DewList’s simple, single-task design is perfect for neurodivergent minds."
    },
    {
      name: "Fiona",
      role: "ADHD Coach",
      quote:"DewList helps me get out of bed on difficult days. It has a beautiful, simple design that's easy to use and stops me getting distracted."
    },
    {
      name: 'Ruth',
      role: "Real Estate Broker",
      quote:"DewList makes it so easy to capture thoughts on the fly. Client follow-ups, personal errands, reminders to take care of myself - everything goes into my Dewlist."
    },
  {
    name: "Davie",
    role: "CS Master's Student",
    quote: "Overall, it's clean. I love the simplicity. Love the interactions. And, above all, I love the fun animations - small details like that make a difference."
  },
  {
    name: "Brandon",
    role: "Sales Representative",
    quote: "I've been hesitant to adopt all this new AI stuff, but DewList's AI feature is almost invisible and makes adding tasks a breeze! The app looks beautiful too."
  },
  {
    name: "Derrick",
    role: "Founder of DewList",
    quote: "DewList is a passion project that started as a simple idea to help myself and my brother get organized. We now use it daily, and I hope it helps you too!"
  }     
    ]